// Stealed from https://github.com/unmojang/drasl/tree/master/ts
...
async function background(el: HTMLDivElement) {
  ...
  console.log('Texture loaded:', endPortalTexture);
  ...
  let shouldAnimate = isSupported && !isFirefox;

  if (isFirefox) {
    console.log('Firefox detected, using CSS fallback');
    el.style.background = 'linear-gradient(45deg, #000 0%, #111 50%, #000 100%)';
  ...
  const handleChangePrm = () => {
    shouldAnimate = isSupported && !isFirefox && !prmQuery.matches;
  };
  handleChangePrm();
